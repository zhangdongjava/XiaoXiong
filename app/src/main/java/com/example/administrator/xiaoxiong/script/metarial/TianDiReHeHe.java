 ...
 */
public class TianDiReHeHe extends BaseStep {
    @Override
    public boolean run() {
        htmlUtil.linkName("返回游戏");
        if(!htmlUtil.getText().contains("和兵阵")){
    ...
    private void guai() {
        LinkBean res = htmlUtil.linkName("小和兵");
        if (!res.isSuccess()) {
            res = htmlUtil.linkName("大和兵");
        }
        if (!res.isSuccess()) {
            return;
        }
