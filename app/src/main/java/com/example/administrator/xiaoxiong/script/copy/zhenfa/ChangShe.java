...
public class ChangShe extends ZhenFa {
    ...
    public boolean fbRun() {
        ...
        for (int i=0;i<1000&&!Thread.currentThread().isInterrupted();i++){
            if (htmlUtil.exitsName("长蛇中心", true)) {
                break;
            } else if (htmlUtil.exitsName("下:长蛇阵↓")) {
                ac = DOWN;
                zhandou("下:长蛇阵↓");
            } else if (htmlUtil.exitsName("左:长蛇阵←")&&ac!=RIGHT) {
                ac = LEFT;
                zhandou("左:长蛇阵←");
            } else if(htmlUtil.exitsName("右:长蛇阵→")&&ac!=LEFT) {
                ac = RIGHT;
                zhandou("右:长蛇阵→");
            }else {
                if(ac ==LEFT){
        ...
        htmlUtil.linkName("长蛇中心", true);
        htmlUtil.linkName("长蛇将军");
