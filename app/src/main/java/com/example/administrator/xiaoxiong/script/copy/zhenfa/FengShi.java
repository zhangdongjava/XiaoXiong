...
public class FengShi extends ZhenFa {
    ...
    public boolean fbRun() {
        ...
        for (int i=0;i<1000&&!Thread.currentThread().isInterrupted();i++){
            if (htmlUtil.exitsName("锋矢中心", true)) {
                break;
            } else if (htmlUtil.exitsName("下:锋矢阵↓")) {
                zhandou("下:锋矢阵↓");
            } else if (htmlUtil.exitsName("左:锋矢阵←")&&ac!=RIGHT) {
                ac = LEFT;
                zhandou("左:锋矢阵←");
            } else if(htmlUtil.exitsName("右:锋矢阵→")&&ac!=LEFT) {
                ac = RIGHT;
                zhandou("右:锋矢阵→");
            }else {
                if(ac ==LEFT){
        ...
        htmlUtil.linkName("锋矢中心", true);
        htmlUtil.linkName("锋矢将军");
